...
# This script works for year 108.
...
dir='y108'
echo 'Starting process year 108'
mkdir $dir
...
seq -w 153 | while read i; do
    ...
    curl -s -o $dir/pdf/star_result-$i.pdf https://www.cac.edu.tw/CacLink/star108/108star_Kresult_3ks8e5/html_108haha/standard/one2seven/$i/108Standard_$i.pdf
...
cat ../school | while read n s; do
    ...
    cat $dir/csv/star_result-$n.csv | while read line; do
        ...
        echo -en "$id\t$recruit\t$first_percent\t$first_ppl\t$second_percent\t$second_ppl\t" >> 108
        echo -e "$s\t$dep" >> 108 
    done
    echo 'Done'
done
