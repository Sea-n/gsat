...
# This script works for year 103.
...
#    curl -s -o pdf/star_result-$i.pdf https://www.cac.edu.tw/cacportal/star_his_report/103/103_result_standard/one2seven/$i/103Standard_$i.pdf
...
cat ../school | while read n s; do
    ...
    cat csv/star_result-$n.csv | while read line; do
        [[ -z `echo $line | cut -d, -f5` ]] && read line
        [[ -z $line ]] && break;
        ...
        echo -en "$id\t$recruit\t$first_percent\t$first_ppl\t$second_percent\t$second_ppl\t" >> 103
        echo -e "$s\t$dep" >> 103
    done
    echo 'Done'
done
